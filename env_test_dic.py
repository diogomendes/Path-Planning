...
class Environment(gym.Env):
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "lidar": spaces.Box(low=-np.inf, high=np.inf, shape=(2 * lidar_points_count + 1,), dtype=np.float32)
            }
        )
    ...
    def get_state(self):
        ...
        for i in state:
            if math.isinf(i):
                state[j] = 100
            j += 1
        if np.any(np.isnan(self.gps.getValues()[:2])):
            ini = self.init_pos
        else:
            ini = np.array(self.gps.getValues()[:2])
        return {
                "agent": np.array(ini, dtype=np.float32),
                "target": np.array(self.final_position, dtype=np.float32),
                "lidar": np.array(state, dtype=np.float32)
            }
    ...
    def get_reward(self):
        ...
        """
        if distance_to_goal < 0.5:  # Muito perto do objetivo
            rew += 10 * (0.5 - distance_to_goal)
        elif distance_to_goal < 1.5:  # Moderadamente perto
            rew += 5 * (1.5 - distance_to_goal)
        elif distance_to_goal < 2.5:  # Relativamente distante
            rew += 2 * (2.5 - distance_to_goal)
        else:  # Muito distante
            rew += 1 * (distance_to_goal - 2.5)
