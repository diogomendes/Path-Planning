...
class Environment(gym.Env):
    def __init__(self):
        ...
        self.bate=False
        self.min_d = self.calculate_distance_to_goal()
    ...
    def step(self, action):
        ...
        self.get_min_distance(n)
        info = {"distance": n,"mean_obs": (self.detect_obstacle_mean(self.lidar.getPointCloud())/self.steps_done), "min_distance":self.min_d}
        if done:
    ...
    def detect_obstacle_mean(self, lidar_point_cloud):
        """
        Detecta a proximidade de obstáculos usando dados do sensor LIDAR.
        ...
        for point in lidar_point_cloud:
            distance_to_robot = np.sqrt(point.x ** 2 + point.y ** 2)
            if not math.isinf(distance_to_robot):
                if distance_to_robot < min_distance:
                    min_distance = distance_to_robot

        return min_distance

    def get_min_distance(self, n):
        if self.min_d > n:
            self.min_d = n
